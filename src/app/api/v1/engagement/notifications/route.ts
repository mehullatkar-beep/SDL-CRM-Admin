...
import { selectPublicNotificationBanners } from "@/lib/public-banners";
...
export async function GET(request: Request) {
  ...
  try {
    const notifications = selectPublicNotificationBanners(await loadBanners());
    return paginatedResponse(request, notifications, parsed.data.page, parsed.data.pageSize);
  } catch (error) {
    logError("engagement.notifications.failed", error, { requestId: requestId(request) });
